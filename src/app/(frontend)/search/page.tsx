...
import { parseTerms, termsWhere, allTermsMatch } from '@/lib/searchTerms'
...
export default async function SearchPage({ searchParams }: Props) {
  ...
  if (query) {
    ...
    // Split the query into words, drop function words, and require every
    // remaining word to match as a whole word (singular or plural) somewhere
    // in title/keywords/imageId. Word order does not matter: "Patara
    // Lighthouse" finds rows with "Patara" in keywords and "Lighthouse" in
    // the title. See src/lib/searchTerms.ts — the AI search tools use the
    // same rules.
    const terms = parseTerms(query)

    const sectionWhere: Where = {
      and: [...termsWhere(terms, ['title', 'keywords']), published],
    }
    ...
    sections = (sectionDocs as Array<{ id: number; title: string; slug: string; sectionType?: string | null; keywords?: string | null }>)
      .filter((s) => allTermsMatch(terms, [s.title, s.keywords]))
      .slice(0, 50)

    const photoWhere: Where = {
      and: [...termsWhere(terms, ['title', 'keywords', 'imageId']), published],
    }
    ...
    photos = (photoDocs as Array<{ id: number; title: string; imageId: string; filename?: string | null; keywords?: string | null }>)
      .filter((p) => allTermsMatch(terms, [p.title, p.keywords, p.imageId]))
      .slice(0, 100)
  }
